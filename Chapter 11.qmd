...
base
```
The appearance is controlled using the `guide_colorbar()` function. The most important arguments are
- `reverse` to flip the color bar to put the lowest values at the top
- `barwidth` and `barheight` to specify the size of the bar, specified by grid units, e.g. `unit(1,'cm')`
...
```
### 11.3.6 Legends
Legends can be customized using the `guide` argument or with the `guides()` helper function and the `guide_legend()` functiodn.
 - `nrow` or `ncol` specifies the dimension of the table. `byrow` controls how the table is filled with `FALSE` being by column and `TRUE` for by row. 
```{r}
#| layout-ncol: 3
#| fig-width: 3
#| fig-height: 3
