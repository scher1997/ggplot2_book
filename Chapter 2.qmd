...
https://aditya-dahiya.github.io/ggplot2book3e/ for other good examples. 
...
One of the bread and butter data sets we will use for visualizations
...
The main two types of plots for time series are line plots `geom_line()` and path plots `geom_path()`

We will use the `economics` dataset since the `mpg` dataset only has two values for the *years* variable. 
...
```
Let's look at the unemployment rate vs length of unemployment and use the `geom_path()` layer to see changes in the rate over time. We will also take advantage of the tip from the book to color the points by the year in time to make it easier to track
```{r}
...
```{r}
ggplot(mpg,aes(cty,hwy))+geom_jitter(alpha=0.3)
```
...
```{r}
ggplot(mpg,aes(reorder(class,hwy),hwy,fill=manufacturer))+geom_boxplot()
```
...
```
Let's zoom in on the less than 2 carat sized ones
```{r}
...
```{r}
# Using geom_histogram() and faceting
...
ggplot(diamonds,aes(x=price,y=cut,color=cut))+
  geom_boxplot()
```
5) What are the strengths and weaknesses of the three ways to compare distributions of subgroups?
- For the `geom_histogram()` and faceting, it's a straightforward idea but then scale-wise you're having to squint to see some of the nuance there. 
- For the `geom_violin()/boxplot()` with color, the color isn't needed since you can just separate them by a y-axis/x-axis. However outliers and such add difficulty in seeing the tails of the distribution. 
- For the `geom_freqplot()` with color, you get it all on one plot as well but the overlaps make it harder to see differences. 
Winner winner chicken dinner for me is a boxplot/violinplot in this scenario. 

6) Read the documentation for `geom_bar()`. What does the `weight` aesthetic do?
```{r}
?stat_count
# weight gives the sum of a continuous value
ggplot(mpg, aes(class)) +
  geom_bar() # Num cars in each class
ggplot(mpg,aes(class))+
  geom_bar(aes(weight=displ)) # Total displacement of each class of cars
```
7) Using the techniques already discussed in this chapter, come up with three ways to visualize a 2D categorical distribution.
```{r}
ggplot(data = mpg,aes(x = class, y = manufacturer)) +
  geom_tile()
```
```{r}
library(ggmosaic)
ggplot(data = mpg) +
  geom_mosaic(aes(x = product(class, manufacturer), fill = manufacturer))

```
```{r}
ggplot(data = mpg,aes(x = hwy)) +
  geom_histogram(bins = 10) +
  facet_grid(cyl ~ trans)

```
## 2.7 Modifying the axes
Just an intro but using `xlab()` and `ylab()` are the most popular ones
```{r}
ggplot(mpg, aes(cty, hwy)) +
...
```
`xlim()` and `ylim()` for limits of axes
```{r}
ggplot(mpg,aes(drv,hwy))+geom_jitter(width=0.2)

ggplot(mpg,aes(drv,hwy))+
  geom_jitter(width=0.2)+
  xlim('f','r')+ # Note it worked for categorical variables
  ylim(20,30)
...
```
## 2.8 Ouput
We can create a plot object, manipulate it (imagine a ``base'' plot), and choose when to show it, as well as save a plot to a file
```{r}
